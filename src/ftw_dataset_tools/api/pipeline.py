...
def stage_download_images(ctx: PipelineContext) -> None:
    ...
    ctx.download_result = download_imagery_for_catalog(
        ...
        # A pipeline run is resumable by definition: every other stage reuses what is
        # already on disk. Without this a rerun re-attempts every chip - and fails on
        # all of them, because a completed download replaces the child's band assets
        # with the local `image`, so there are no band hrefs left to fetch.
        resume=True,
    )
