...
from ftw_dataset_tools.api.geo import reproject


class TestReprojectUTMToWGS84:
    """Tests for reprojecting from UTM to WGS84."""

    def test_reproject_point_utm_to_wgs84(self, create_geoparquet, tmp_path):
        ...
        assert "32610" in result.source_crs or "UTM" in result.source_crs.upper()
...
class TestAxisOrder:
    ...
    def test_output_uses_lonlat_order(self, create_geoparquet, tmp_path):
        ...
        reproject(input_file=input_file, output_file=output_file)
...
class TestPolygonReprojection:
    ...
    def test_reproject_polygon(self, create_geoparquet, tmp_path):
        ...
        result = reproject(input_file=input_file, output_file=output_file)
...
class TestOverwriteMode:
    ...
    def test_reproject_overwrite(self, create_geoparquet):
        """Test reprojection with overwrite flag (output_file == input_file)."""
        # Create input file
        input_file = create_geoparquet(
        ...
        result = reproject(
            input_file=input_file,
            output_file=input_file,  # Same as input
            target_crs="EPSG:4326",
        )
...
class TestMultipleFeatures:
    ...
    def test_reproject_multiple_features(self, duckdb_conn, tmp_path):
        ...
        """
        duckdb_conn.execute(f"COPY ({query}) TO '{input_file}' (FORMAT PARQUET)")
